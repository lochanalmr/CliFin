...
Every database initialization, connection, and writing is transparently notified. Some processes may trigger multiple database initializations. At every transaction/asset/subscription entry, database is being initialized, as the program itself holds no memory of any occurences happened in the program during runtime.
...
`subscriptions.db` contains subscriptions data.